...
import { ScrollView,ToastAndroid, Alert, StyleSheet, Text, TextInput, Pressable, Button, View, Image, TouchableOpacity, ActivityIndicator, Modal } from 'react-native'
...
export default function Police({ navigation, route }) {
        ...
        axios.post(`${BASE_URL}request`, objToSend)
            .then((res) => {
                ...
                if (res.data.status) {
                    ...
                    ToastAndroid.show(res.data.message.message, ToastAndroid.SHORT)
 ...
 <Modal
            ...
            <View style={[styles.bgWhite,styles.w100,styles.p3, ]}>
                <TouchableOpacity onPress={() => setModalVisible(true,setRequestType("Fight"))} style={[styles.bgRed, styles.w100, styles.p1, styles.flexCenter, styles.mb2, styles.px3, { borderRadius: 50 }]}>
                    <Text style={[styles.textLight, styles.fs5]}>{isloading ? <ActivityIndicator color={styles._white} size={"small"} /> : "Fight / Fire"}</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => setModalVisible(true,setRequestType("Robbery"))} style={[styles.bgRed, styles.w100, styles.p1, styles.flexCenter, styles.mb2, styles.px3, { borderRadius: 50 }]}>
                    <Text style={[styles.textLight, styles.fs5]}>{isloading ? <ActivityIndicator color={styles._white} size={"small"} /> : "Robbery"}</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => setModalVisible(true,setRequestType("Harassment"))} style={[styles.bgRed, styles.w100, styles.p1, styles.flexCenter, styles.mb2, styles.px3, { borderRadius: 50 }]}>
                    <Text style={[styles.textLight, styles.fs5]}>{isloading ? <ActivityIndicator color={styles._white} size={"small"} /> : "Harassment"}</Text>
                </TouchableOpacity>
            </View>
